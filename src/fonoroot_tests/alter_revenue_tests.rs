...
#[test]
fn alter_revenue_table_possible_to_alter_values() {
    ...
    let mut catalogue_of_alice = contract.catalogues.get(&accounts(0)).unwrap();
    let mut inserted_revenue_table = catalogue_of_alice.get(&0).unwrap().unwrap();
    let mut income_table = contract.income_tables.get(&0).unwrap();
    ...
    new_unchecked_table.insert(accounts(0), 7000);
    new_unchecked_table.insert(accounts(1), 3000);

    // Modify the RevenueTable
    id = alter_revenue_table_proposal(&mut context, &mut contract, 0, new_unchecked_table, U128(to_yocto("3")));
    contract.act_proposal(id, Action::VoteApprove, None);

    catalogue_of_alice = contract.catalogues.get(&accounts(0)).unwrap();
    inserted_revenue_table = catalogue_of_alice.get(&0).unwrap().unwrap();
    income_table = contract.income_tables.get(&0).unwrap();

    assert_eq!(inserted_revenue_table.revenue_table.get(&accounts(0)), Some(&7000), "Alice should have 70% of the revenue of the newly minted song.");
    assert_eq!(inserted_revenue_table.revenue_table.get(&accounts(1)), Some(&3000), "Bob should have 30% of the revenue of the newly minted song.");

    assert_eq!(income_table.price, Some(U128(to_yocto("3"))), "The price of the new NFT should be 3 NEAR now.");
}

/// This should panic, because the caller is not owner
#[test]
#[should_panic(expected = "Only the owner (Artist) can alter the revenue table!")]
fn alter_revenue_table_not_owner_error() {
    let mut context = VMContextBuilder::new();
    testing_env!(context.predecessor_account_id(accounts(1)).build());                          // This is Bob
    let mut contract = Contract::new(
        Config::test_config(),
    ...
    // Bob is trying to alter the RevenueTable
    id = alter_revenue_table_proposal(&mut context, &mut contract, 0, new_unchecked_table, U128(to_yocto("3")));
    contract.act_proposal(id, Action::VoteApprove, None);
}
...
fn alter_revenue_table_table_not_correct() {
    let mut context = VMContextBuilder::new();
    testing_env!(context.predecessor_account_id(accounts(1)).build());                          // This is Bob
    let mut contract = Contract::new(
        Config::test_config(),
    ...
    let mut new_unchecked_table = HashMap::default();
    new_unchecked_table.insert(accounts(0), 8000);
    new_unchecked_table.insert(accounts(1), 3000);
