...
use crate::fonoroot_tests::helpers::{
    ...
    prepare_nft_full_proposal,
    prepare_nft_image_hash_missing_proposal,
    prepare_nft_meta_hash_missing_proposal,
...
/// Prepare full data tests (All the NFT data is filled, ready to be minted)
#[test]
fn prepare_data_ready_tests() {
    ...
    id = prepare_nft_full_proposal(&mut context, &mut contract);
    contract.act_proposal(id, Action::VoteApprove, None);
    ...
    assert_eq!("Description about a Test NFT".to_string(), the_inserted_data.desc.unwrap(), "The Description is not correct!");
    ...
    assert_eq!("QmeCBSWQcDwn3ktKdEfDn68bt2PQbx3khyqGQAeYAVUHpb".to_string(), the_inserted_data.meta.unwrap(), "The Meta JSON CID is not correct!");
    assert_eq!(Base64VecU8(vec![10,11,12]), the_inserted_data.meta_hash.unwrap(), "The Meta JSON hash is not correct!");
    ...
    // Add 1 more InProgressNft      
    id = prepare_nft_full_proposal(&mut context, &mut contract);
    contract.act_proposal(id, Action::VoteApprove, None);
    
    assert_eq!(2, contract.in_progress_nonce, "The in_progress_nonce should be 2 at this point.");
}

/// This test is also with full data (All the NFT data is filled, ready to be minted), but the initiator is not in the master group for this contract
#[test]
#[should_panic(expected = "You are not allowed to mint on this specific contract.")]
fn prepare_data_not_allowed() {
    let mut context = VMContextBuilder::new();
    testing_env!(context.predecessor_account_id(accounts(1)).build());                          // This is Bob
    let mut contract = Contract::new(
        Config::test_config(),
    ...
    // Prepare NFT
    id = prepare_nft_full_proposal(&mut context, &mut contract);                                // Artist is Bob
    contract.act_proposal(id, Action::VoteApprove, None);
}

/// This should fail, because the frond end is trying to add an image CID to the InProgressMetadata, without a hash
#[test]
#[should_panic(expected = "Hash has to exist, if image exists!")]
fn prepare_data_image_hash_error() {
    let mut context = VMContextBuilder::new();
    testing_env!(context.predecessor_account_id(accounts(1)).build());                          // This is Bob
    let mut contract = Contract::new(
        Config::test_config(),
    ...
    // Prepare NFT
    testing_env!(context.predecessor_account_id(accounts(0)).build());                          // Artist will be Alice
    id = prepare_nft_image_hash_missing_proposal(&mut context, &mut contract);
    contract.act_proposal(id, Action::VoteApprove, None);
}

/// This should fail, because the frond end is trying to add a meta to the InProgressMetadata, without a hash
#[test]
#[should_panic(expected = "Hash has to exist, if meta exists!")]
fn prepare_data_meta_hash_error() {
    let mut context = VMContextBuilder::new();
    testing_env!(context.predecessor_account_id(accounts(1)).build());                          // This is Bob
    let mut contract = Contract::new(
        Config::test_config(),
    ...
    // Prepare NFT
    testing_env!(context.predecessor_account_id(accounts(0)).build());                          // Artist will be Alice
    id = prepare_nft_meta_hash_missing_proposal(&mut context, &mut contract);
    contract.act_proposal(id, Action::VoteApprove, None);
}

/// Prepare half-ready data tests (Not read to be minted)
#[test]
fn prepare_data_half_tests() {
    let mut context = VMContextBuilder::new();
    testing_env!(context.predecessor_account_id(accounts(1)).build());                          // This is Bob
    let mut contract = Contract::new(
        Config::test_config(),
    ...
    // This is all arbitrary data from front end
    assert_eq!("Test NFT".to_string(), the_inserted_data.title.unwrap(), "The Title is not correct!");
    assert!(the_inserted_data.desc.is_none(), "The Description field should be empty!");
    assert_eq!(AccountId::new_unchecked("minting-contract-1.near".to_string()), the_inserted_data.contract, "The Minting Contract is not correctly set!");
    assert_eq!("QmerincKVRPTXh1z41725mFNvGp31UBgfyms5xWi1taNuQ".to_string(), the_inserted_data.image.unwrap(), "The Image CID is not correct!");
    assert_eq!(Base64VecU8(vec![1,2,3]), the_inserted_data.image_hash.unwrap(), "The Image hash is not correct!");
    assert_eq!("QmU51uX3B44Z4pH2XimaJ6eScRgAzG4XUrKfsz1yWVCo6f".to_string(), the_inserted_data.music.unwrap(), "The Music Folder CID is not correct!");
    ...
    assert_eq!("https://ipfs.io/ipfs/QmU51uX3B44Z4pH2XimaJ6eScRgAzG4XUrKfsz1yWVCo6f".to_string(), the_inserted_data.animation_url.unwrap(), "The Animation URL is not correct!");
    assert_eq!(Base64VecU8(vec![7,8,9]), the_inserted_data.animation_url_hash.unwrap(), "The Animation URL hash is not correct!");
    assert!(the_inserted_data.meta.is_none(), "The Meta JSON CID field should be empty");
    assert!(the_inserted_data.meta_hash.is_none(), "The Meta JSON hash field should be empty!");
